...
PathMatchC_initialize(VALUE self, VALUE rb_path, VALUE rb_oQuery)
{
  ...
  if (query_len == 0) {
    score = 1.0;
  } else if (path_len > 0 && query_len <= path_len) {
    pathmatch_t pm;
    pm.query = query;
    ...
    memset(pm.qc_pos_map, '\0', POS_MAP_SIZE * sizeof(pos_list_t));
...
Init_pathmatchc()
{
  ...
  // def initialize(path_str, query_obj)
  ...
  // statically to avoid repeated calls into Ruby.
